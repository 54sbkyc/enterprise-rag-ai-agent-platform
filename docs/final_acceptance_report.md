...
| 质量评测 | 已完成 | 50 条分层黄金用例按角色执行，严格检查事实、来源、拒答、访问控制、引用忠实、安全断言和执行证据。 |
...
| CI | 已完成 | GitHub Actions 运行 175 项 pytest、2 项真实 pgvector PostgreSQL 集成测试、RAG 门禁和容器构建/登录/重启烟测。 |
...
- 模型重试、熔断和计数器当前是单进程状态；多实例生产环境需要独立模型代理或共享网关，并接入集中 tracing 和告警。
...
```text
这个项目是我在毕业设计基础上继续升级出的 AI 应用开发作品集。它不是简单 ChatGPT 套壳，而是围绕企业知识库问答做了文档入库、权限过滤、RAG 检索、引用溯源、Agent 工具调用、安全审计、质量评测和 AI 可观测。为了让项目可以复现和交付，我还补了 175 项自动化测试、2 项真实 pgvector PostgreSQL 集成测试、50 条严格 RAG 质量门禁、版本化数据库迁移、非 root 容器、容器 CI 实跑、发布清单和最终验收报告。
```
