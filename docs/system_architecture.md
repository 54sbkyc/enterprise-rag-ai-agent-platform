...
系统采用前后端分离思想实现，本地部署时由 FastAPI 同时提供接口服务和静态前端页面。SQLite 保存业务事实并保证零服务启动；大语料场景可叠加 PostgreSQL + pgvector HNSW 向量检索投影。
...
flowchart LR
    ...
    A --> Audit["审计日志模块"]
    Auth --> DB["SQLite 数据库"]
    Search -. "可选向量 Top-K" .-> PG["PostgreSQL + pgvector"]
    Doc -. "可选向量同步" .-> PG
    Doc --> Store["上传文件目录"]
    Doc --> DB
    Search --> DB
...
| 检索模块 | 使用 BM25、可选 SQLite/pgvector 向量相似度和本地重排实现 Top-K 检索。 |
...
- SQLite 和本地文件存储降低默认部署复杂度；可选 pgvector Profile 与真实数据库 CI 证明向量通道具备扩展路径。
