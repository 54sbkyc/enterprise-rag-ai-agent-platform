...
当前已经实现正文 70%、标题 30% 的字段加权 BM25、本地重排和可选 Embedding 混合召回。默认 SQLite JSON 保证仓库零服务启动并承担故障降级；可选 pgvector 使用连接池与 HNSW 做真实向量 Top-K。查询先由 SQLite 根据当前角色生成允许片段 ID，再交给 pgvector 排序，避免索引延迟造成越权。检索调试会展示 BM25、Vector、Rerank、最终分数、实际向量后端和降级状态。
