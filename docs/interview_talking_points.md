...
系统支持单条评测和版本化批量门禁。v3 黄金集的 50 条用例按角色、难度和能力分层执行；严格事实组、替代来源组、禁答词、禁用文档和最小引用数分别约束生成、召回和安全。系统计算 Recall@K、MRR、答案、拒答、访问控制、引用忠实与安全断言七项门禁指标，同时记录模型、Prompt、Token、成本和耗时。应用优先比较兼容历史运行，CI 固定比较批准基线；CLI 失败返回非零退出码，因此 GitHub Actions 能真正阻止质量退化，而不只是展示报表。
...
- 实现 50 条分层角色化黄金集、七项质量与安全门禁、分层诊断和模型执行证据，并保留知识缺口、反馈闭环和审计日志。
