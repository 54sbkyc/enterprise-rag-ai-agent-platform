...
### 为什么先用 SQLite 保存向量，而不是直接上向量数据库？

当前已经实现正文 70%、标题 30% 的字段加权 BM25、本地重排和可选 Embedding 混合召回。字段权重解决了通用流程词压过精确文档标题的问题；为了保持仓库一键运行，向量先保存在 SQLite JSON 字段中，语料规模扩大后再迁移到 pgvector。检索调试页会分别显示 BM25、Vector、Rerank 和最终分数。
...
系统有管理员和普通员工角色。后端接口使用权限检查，文档检索会根据用户角色过滤可访问密级。前端也会隐藏无权限页面，但安全边界主要在后端，避免只靠 UI 隐藏。

### 如何评估回答质量？

系统支持单条评测和版本化批量门禁。黄金用例与批准基线进入 Git 并计算 SHA-256 指纹，分别计算 Recall@K、MRR、答案正确率和拒答准确率，避免把召回失败和生成失败混成一个分数。应用优先比较兼容历史运行，CI 固定比较批准基线；CLI 失败返回非零退出码，因此 GitHub Actions 能真正阻止质量退化，而不只是展示报表。
...
- 实现版本化黄金集、Recall@K、MRR、答案/拒答准确率与 CI 回归门禁，并保留知识缺口、反馈闭环和审计日志。
...
- 当前已支持 BM25 + 可选 Embedding 混合检索，但向量仍存储在 SQLite，不适合大规模语料。
