...
| 评测中心 | 展示单条评测、批量评测、引用命中率和报告导出，说明如何评估回答质量 |
...
当前已经实现 BM25、本地重排和可选 Embedding 混合召回。为了保持仓库一键运行，向量先保存在 SQLite JSON 字段中；语料规模扩大后再迁移到 pgvector。检索调试页会分别显示 BM25、Vector、Rerank 和最终分数。
...
系统支持单条评测和批量评测。批量评测分别计算 Recall@K、MRR、答案正确率和拒答准确率，避免把召回失败和生成失败混成一个分数。结果可以导出 Markdown/CSV。
...
- Agent 已支持超时重试和状态持久化，生产环境还需要异步队列、取消、幂等、人工审批和更完整的权限策略。
